...
рішення/дії/відкриті питання та окремо контекст. Для зустрічей від 10 хвилин
додатковий closing-pass повторно перевіряє останні 8 хвилин, де найчастіше
звучать фінальні домовленості. Після цього текстовий reasoning-прохід
хронологічно перевіряє прийняття, заперечення й пізніші зміни, а окремий прохід
без reasoning формує валідний JSON. Фінальний summary будується лише з
узгодженого ledger.

У `Рішення` потрапляють лише явно підтверджені домовленості; поради й неприйняті
варіанти позначаються як пропозиції в тезах. У `Відкритих питаннях` лишаються
тільки справжні питання без відповіді. У `Action items` потрапляють відкриті
зобов'язання; невідомий owner показується явно, а не як порожнє поле. Надіслане
під час зустрічі посилання чи контакт не створює задачу в Notion.

Кожна evidence-цитата отримує детермінований номер рядка та час із транскрипту.
Непідтверджені числа й дні тижня відкидаються, семантичні дублікати
об'єднуються, а перед записом summary запускається quality gate для рішень і
дублікатів. Перевірені підстави та попередження зберігаються приватно у
...
| `SUMMARY_CONTEXT_NUM_PREDICT` | ліміт контекстного evidence JSON; default `4096` |
| `SUMMARY_CONTEXT_REPAIR_ITEMS` | максимум items у компактному retry після пошкодженого JSON; default `5` |
...
# повторно застосувати дедуплікацію/quality gate без нового виклику Ollama
.venv/bin/python3 watch_and_process.py --refresh-summary-render SESSION
```
