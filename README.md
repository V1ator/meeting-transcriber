  ...
  └─ record.py
       ...
       └─ atomic session manifest
            └─ watch_and_process.py
                 ...
                 8. ротація сирих WAV через 5 днів
```

Двотрекова архітектура дозволяє позначати вашу доріжку як «Я» без діаризації.
Обидва режими безпечно записують мікрофон у Raw і не змінюють його рівень у
Zoom/Meet. «Динаміки» додатково вмикає локальну діаризацію кількох людей і
дедуплікацію із системною доріжкою.
...
- Хоткей або SwiftBar → вибір профілю «Навушники» чи «Динаміки»; запис мікрофона в обох випадках Raw.
- Повторний виклик коректно закриває обидві доріжки та створює manifest зі станом `recorded`.
- Якщо обидві доріжки тихі 90 секунд, з'являється popup із пропозицією завершити запис. Якщо відповіді немає, запис безпечно зупиняється після 5 хвилин безперервної тиші. «Продовжити» відкладає наступне нагадування на 10 хвилин.
...
- «Навушники» запускає Raw-запис і позначає локальну доріжку як «Я»;
- «Динаміки» також використовує Raw, але діаризує людей біля локального мікрофона (`LOCAL_00`, `LOCAL_01`) і прибирає дублікати із системною доріжкою;
...
Прямий `toggle_record.sh --raw` або `toggle_record.sh --speakers` лишається для
діагностики та інтеграцій, яким уже надано доступ до мікрофона.
Старий `--aec` підтримується як сумісний псевдонім `--speakers`, але live-AEC
...
| `RECORD_MIC_MODE` | `single` | fallback-профіль: `single/headphones` або `multiple/speakers`; capture завжди Raw |
| `DEDUP_MIC` | `true` | консервативна дедуплікація мікрофонної та системної доріжок |
...
| `SILENCE_MIN_RECORD_SECONDS` | `90` | не показувати popup раніше 90 секунд після старту |
| `SILENCE_REPEAT_SECONDS` | `600` | інтервал повторного нагадування |
| `SILENCE_DIALOG_TIMEOUT` | `30` | час на відповідь; після timeout вмикається відкладений auto-stop |
| `SILENCE_AUTO_STOP_SECONDS` | `300` | auto-stop після timeout popup і 5 хвилин безперервної тиші |
| `MIC_ACTIVITY_DBFS` | `-42` | поріг активності мікрофона |
| `SYSTEM_ACTIVITY_DBFS` | `-50` | поріг активності системної доріжки |
| `ROTATE_DAYS` | `5` | вік WAV до видалення; `0` — не видаляти |
...
- **Повільна діаризація:** перевірте `DIARIZE_DEVICE=auto`, доступність MPS і `~/Library/Logs/MeetingTranscriber/watcher.log`.
- **Дублікати без навушників:** у стартовому popup обирайте «Динаміки», щоб увімкнути локальну діаризацію та дедуплікацію.
...
- У режимі «Динаміки» звук співрозмовників може потрапляти також у мікрофонну доріжку; консервативна дедуплікація прибирає не всі випадки.
