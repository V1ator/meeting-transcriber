...
пакети стабільно не декодуються або відкритий канал втрачено, автоматичний DOM
fallback не запускається: резервом є локальний запис аудіо. Відсутність
мовлення у справному відкритому каналі не вважається помилкою. Старий DOM-код
залишений лише як явно ввімкнений діагностичний режим (`domFallbackEnabled` у
локальних settings розширення); за замовчуванням він вимкнений. Оновлення однієї
RTC-репліки збираються за `messageId` і `messageVersion`, тому старі версії не
дублюють текст.
