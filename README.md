...
1. У Meet переконайтеся, що віджет показує **«Запис RTC captions»**.
2. Вмикати CC у Meet не потрібно: розширення не торкається native captions і
   не змінює геометрію відеосцени.
3. Якщо RTC не підключився за 20 секунд або втрачений посеред зустрічі, віджет
   покаже червоне попередження. Натисніть **«Запустити аудіозапис»** і
   підтвердьте режим у системному вікні.
...
пакети стабільно не декодуються або відкритий канал втрачено, резервом є
локальний запис аудіо. Відсутність мовлення у справному відкритому каналі не
вважається помилкою. Розширення взагалі не читає captions із DOM, не приховує CC
і не змінює розмітку Meet; DOM використовується лише для доступного чату,
учасників і зіставлення RTC device ID з іменем. Оновлення однієї RTC-репліки
